...
class BookDataRepository:
    ...
    async def get_book_by_id(self, book_id: int) -> Book | None:
        cursor = await self._session.execute(
            select(book_table.c["id", "title", "author", "chapter_number", "added_time"]).where(
                book_table.c.id == book_id))
    ...
    async def add_book(self, book: dto.BookForUploadDTO):
        await self._session.execute(
            insert(book_table).values({"title": book.book_title, "author": book.book_author, "chapter_number": 1, }))
